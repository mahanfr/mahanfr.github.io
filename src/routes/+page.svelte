<script>
    export let data;
    import image from '$lib/assets/mahanfr.jpg'
    import books from '$lib/json/book-recommend.json'
</script>

<svelte:head>
    <title>Mahan Farzaneh</title>
    <meta name="description" content="Computer Engineer on Paper, Software Developer and Writer(not really) on the Internet. Love to work on low level stuff and learn every thing about day to day technologies that we use without thinking about it."/>
</svelte:head>

<section class="flex flex-row items-center justify-between mx-4 lg:mx-10 py-3">
    <div class="flex flex-row items-center">
        <img class="rounded-full" src={image} alt="Mahan Farzaneh - ماهان فرزانه" width={"60px"} height={"60px"} />
        <div class="mx-4">
            <h3 class="text-gray-200 text-md">Mahan Farzaneh</h3>
            <h4 class="text-gray-400 text-sm">Computer Engineere</h4>
        </div>
    </div>
    <div class="flex flex-row space-x-2">
    <a href="https://twitter.com/mahan.farzaneh">
        <svg class="bg-gray-300 h-8 w-8 p-1 rounded-full hover:bg-primary" height="56.693px" id="Layer_1" version="1.1" viewBox="0 0 56.693 56.693" width="56.693px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M52.837,15.065c-1.811,0.805-3.76,1.348-5.805,1.591c2.088-1.25,3.689-3.23,4.444-5.592c-1.953,1.159-4.115,2-6.418,2.454  c-1.843-1.964-4.47-3.192-7.377-3.192c-5.581,0-10.106,4.525-10.106,10.107c0,0.791,0.089,1.562,0.262,2.303  c-8.4-0.422-15.848-4.445-20.833-10.56c-0.87,1.492-1.368,3.228-1.368,5.082c0,3.506,1.784,6.6,4.496,8.412  c-1.656-0.053-3.215-0.508-4.578-1.265c-0.001,0.042-0.001,0.085-0.001,0.128c0,4.896,3.484,8.98,8.108,9.91  c-0.848,0.23-1.741,0.354-2.663,0.354c-0.652,0-1.285-0.063-1.902-0.182c1.287,4.015,5.019,6.938,9.441,7.019  c-3.459,2.711-7.816,4.327-12.552,4.327c-0.815,0-1.62-0.048-2.411-0.142c4.474,2.869,9.786,4.541,15.493,4.541  c18.591,0,28.756-15.4,28.756-28.756c0-0.438-0.009-0.875-0.028-1.309C49.769,18.873,51.483,17.092,52.837,15.065z"/></svg> 
    </a>
    <a href="https://linkedin.com/in/mahan-farzaneh">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 w-full gap-2">
        ...
        <div class="border-2 border-secondary p-2 flex flex-row items-center">
            <div class="text-gray-300 mx-2">
                ...
                <div class="flex flex-col md:flex-row mt-2 space-y-2 md:space-y-0 text-center md:space-x-2">
                    <a href="https://github.com/mahanfarzaneh2000/SubLearn" class="hover:bg-secondary text-sm border border-primary rounded-sm px-2">Gitub</a>
                    ...
                    <a href="https://github.com/mahanfarzaneh2000/SubLearn/releases/download/v1.0.0/sublearn_1.0.0_x64_en-US.msi" class="hover:bg-secondary text-sm border border-primary rounded-sm px-2">Download for Deb</a>
                </div>
            </div>
        </div>
        <div class="border-2 border-secondary p-2 flex flex-row items-center">
            <div class="text-gray-300 mx-2">
                <h1 class="font-semibold">Zillojs</h1>
                ...
                <div class="flex flex-row mt-2 space-x-2">
                    <a href="https://github.com/mahanfarzaneh2000/ZilloJs" class="hover:bg-secondary text-sm border border-primary rounded-sm px-2">Gitub</a>
                    <a href="https://www.npmjs.com/package/zillojs" class="hover:bg-secondary text-sm border border-primary rounded-sm px-2">npm</a>
                </div>
            </div>
        </div>
    </div>
...
<section class="border-2 border-secondary mt-2 text-gray-300 p-3 mx-4 lg:mx-10 py-3">
    <h3 class="font-semibold">Book Recommendations</h3>
        {#each books as book}
        <div class="mt-2 p-3 border-2 border-secondary">
            <h3>{book.name} - {book.author}</h3>
            <p class="font-semibold text-gray-400">{book.description}</p>
            <p class="text-sm text-gray-400">
                {#each book.text as txt}
                    <span>{txt}</span>
                    <span> </span>
                {/each}
            </p>
            <p dir="rtl" class="text-sm text-gray-400">
                {#each book.text_fa as txt}
                    <span>{txt}</span>
                    <span> </span>
                {/each}
            </p>
...
<section class="border-2 border-secondary mt-2 min text-gray-300 p-3 mx-4 lg:mx-10 py-3">
    <h3 class="font-semibold">Articles</h3>
        {#each data.articles as article}
        <div class="mt-2 p-3 border-2 border-secondary">
            <a class="flex flex-row justify-between" href={article.path}>
                <h3 class="font-bold" >{article.meta.title}</h3>
                <h3 dir="rtl" class="font-bold" >{article.meta.titlefa}</h3>
            </a>
            <small class="font-semibold text-gray-400">Published at: {article.meta.date}</small>
            <p class="text-gray-400 text-sm">{article.meta.description}</p>
            <p class="text-gray-400 text-md" dir="rtl">{article.meta.descriptionfa}</p>
        </div>
        {/each}
</section>
